class V1::UsersController < ApiApplicationController
  ...
  def can_execute
    ...
    render_unauthorized unless (@current_user.role == User::ROLES[:admin][:id]) or
              (@user and @user.id == @current_user.id) or
              (@current_user.role == User::ROLES[:venue_admin][:id] and (@user == nil or @user.venue_id == @current_user.venue_id)) or
              (params[:action] == :show and @user and @user.id == @current_user.id)
  end
